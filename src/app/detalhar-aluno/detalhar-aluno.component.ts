...
export class DetalharAlunoComponent implements OnInit {
  ...
  editaAluno(id: number) {
    this.alunosService.editaAluno(this.aluno.id, this.aluno).subscribe(
      (aluno) => {
  ...
  DeletaAluno(id: number) {
    this.alunosService.deletaAluno(this.aluno.id).subscribe(
      (aluno) => {
  ...
  uploadFoto() {
    ...
    this.alunosService.addFoto(formData).subscribe(
      (aluno) => {
